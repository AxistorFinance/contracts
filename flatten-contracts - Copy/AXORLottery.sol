...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Axistor Lottery Ticket", "AXORLT") {}
...
// File: contracts\AXORLottery.sol
...
contract Lottery is OwnableUpgradeable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external inDrawingPhase{
        ...
        axor.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external inDrawingPhase {
        ...
        axor.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            safeaxorTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            safeaxorTransfer(address(msg.sender), totalReward);
        }
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        axor.safeTransfer(address(msg.sender), _amount);
