...
// File: @axistor/matrixswap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: contracts/AxistorProfile.sol

pragma solidity ^0.6.0;

/** @title AxistorProfile.
@dev It is a contract for users to bind their address 
to a customizable profile by depositing a NFT.
*/
contract AxistorProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public axorToken;
    ...
    uint256 public numberAxorToReactivate;
    uint256 public numberAxorToRegister;
    uint256 public numberAxorToUpdate;
    ...
    constructor(
        IBEP20 _axorToken,
        uint256 _numberAxorToReactivate,
        uint256 _numberAxorToRegister,
        uint256 _numberAxorToUpdate
    ) public {
        axorToken = _axorToken;
        numberAxorToReactivate = _numberAxorToReactivate;
        numberAxorToRegister = _numberAxorToRegister;
        numberAxorToUpdate = _numberAxorToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends AXOR to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer AXOR tokens to this contract
        axorToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberAxorToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer AXOR token to this address
        axorToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberAxorToUpdate
    ...
    /**
     * @dev Claim AXOR to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        axorToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of AXOR to register
     * Callable only by owner admins.
     */
    function updateNumberAxor(
        uint256 newNumberAxorToReactivate,
        uint256 _newNumberAxorToRegister,
        uint256 _newNumberAxorToUpdate
    ) external onlyOwner {
        numberAxorToReactivate = newNumberAxorToReactivate;
        numberAxorToRegister = _newNumberAxorToRegister;
        numberAxorToUpdate = _newNumberAxorToUpdate;
    }
